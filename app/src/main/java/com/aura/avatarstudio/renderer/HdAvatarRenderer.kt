...
class HdAvatarRenderer(
    ...
    fun updateAppearance(skinTone: String, eyeColor: String, hairColor: String, atmosphere: String) {
        ...
        // 2. Avatar Colors
        val skinRgb = when (skinTone) {
            "Pale/Vampiric" -> floatArrayOf(0.95f, 0.92f, 0.92f)
            "Fair" -> floatArrayOf(0.95f, 0.8f, 0.7f)
            "Tan" -> floatArrayOf(0.75f, 0.55f, 0.45f)
            "Olive" -> floatArrayOf(0.6f, 0.5f, 0.35f)
            "Brown" -> floatArrayOf(0.4f, 0.25f, 0.15f)
            "Ebony" -> floatArrayOf(0.15f, 0.08f, 0.05f)
            else -> floatArrayOf(1.0f, 1.0f, 1.0f)
        }

        val hairRgb = when (hairColor) {
            "Crimson Red" -> floatArrayOf(0.8f, 0.0f, 0.0f)
            "Pink" -> floatArrayOf(1.0f, 0.4f, 0.7f)
            "Black", "Obsidian Black" -> floatArrayOf(0.05f, 0.05f, 0.05f)
            "Blonde", "Platinum Blonde" -> floatArrayOf(0.9f, 0.85f, 0.5f)
            "Blue" -> floatArrayOf(0.1f, 0.3f, 0.8f)
            "Whiteout" -> floatArrayOf(1.0f, 1.0f, 1.0f)
            else -> null // Default
        }

        // Apply dummy material overrides (in a real app, this would target specific mesh names/materials)
        // Here we just tint all meshes slightly to simulate it, prioritizing the primary mesh as skin.
        for ((index, mesh) in av.meshes.withIndex()) {
            if (index == 0) {
                // Assume first mesh is body/skin
                mesh.material.baseColor[0] = skinRgb[0]
                mesh.material.baseColor[1] = skinRgb[1]
                mesh.material.baseColor[2] = skinRgb[2]
            } else if (index == 1 && hairRgb != null) {
                // Assume second mesh is hair
                mesh.material.baseColor[0] = hairRgb[0]
                mesh.material.baseColor[1] = hairRgb[1]
                mesh.material.baseColor[2] = hairRgb[2]
            }
        }
    }
